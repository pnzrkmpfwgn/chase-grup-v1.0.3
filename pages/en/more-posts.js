...
/* I know this looks extremely stupid and actually it's ¯\_(ツ)_/¯ */
  const [data,setData]=useState({items:enData.slice(0,4)})
const fetchMore=()=>{
  const initial=4
  setTimeout(()=>{
    setData({
  ...
  return <div className={styles.posts}>
   ...
   {typeof enData != "undefined" ? (
              <InfiniteScroll dataLength={enData.length - 1} next={fetchMore} hasMore={enData[enData.length]===undefined ? false :true } loader={<Loading/>} endMessage={<p style={{marginTop:"20px",color:"#e58c17",fontSize:"24px",fontWeight:"bold"}}> End of entries... </p>} >
                {enData.map((post) => (
                <div className={styles.post} key={post.id}>
                  ...
                  <div styles={styles.post_stats}>
                    <i className={"far fa-clock"}style={{ marginRight: "10px",color:"white" }}> {date(post.created_at,"en")}</i>
...
export const getStaticProps=async()=>{
  const en_data = await fetch(en_posts_url).then(res => res.json()).then(data=>data)
